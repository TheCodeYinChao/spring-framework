	...
	public void registerBeanDefinition(String beanName, BeanDefinition beanDefinition)
		...
		if (existingDefinition != null) {
			...
			if (hasBeanCreationStarted()) {
				...
				/**
				 * 这里如果我们之传进来一个类，那则默认会有七个类 排除我们自己定义的由6个核心的类型
				 * 比如环境  ， 后置处理器等
				 */
				this.beanDefinitionMap.put(beanName, beanDefinition);//bd集合有啦
				this.beanDefinitionNames.add(beanName);//bd名字有啦  原料都有啦
				removeManualSingletonName(beanName);//移除重复的beanName
			}
